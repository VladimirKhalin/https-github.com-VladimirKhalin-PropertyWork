...
class GTA_GameModesInfoViewController: GTAModes_NiblessViewController {
...
extension GTA_GameModesInfoViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.downloadAction = { [weak self] in
            if GTA_NetworkStatusMonitor.shared.isNetworkAvailable {
                self?.model.gta_downloadMode(index: self?.currentIndex ?? 1)
            } else {
...
extension GTA_GameModesInfoViewController: UIPopoverPresentationControllerDelegate {
    func adaptivePresentationStyle(for controller: UIPresentationController) -> UIModalPresentationStyle {
        return .none
    }
}

extension GTA_GameModesInfoViewController: UIDocumentPickerDelegate, UINavigationControllerDelegate {
